  ...
  { id: 'child-leo', name: 'Leo', age: 8, initial: 'L', isNew: true, intake: {} },
  {
    ...
    intake: {
      ...
      questionnaireAnswers: {
        ...
        sleep: 'After school',
        learning: 'Starting or finishing tasks',
        movement_coordination: 'Attention and concentration',
        speech_communication: 'Gets upset or frustrated',
        school_participation: 'Participates with some support',
        friendships: 'Enjoys playing but has occasional conflicts',
        dev_available_information: ['Teacher observations', 'School reports'],
      },
      completedQuestionnaireSections: QUESTIONNAIRE_SECTIONS,
    },
  },
...
const DEMO_DATA_VERSION = 'quarter-zero-noah-v12-ava-choose-path-nick-v3-mocked';

const CANONICAL_CHILDREN_BY_ID: Record<string, Child> = {
  'child-maya': INITIAL_CHILDREN[5],
  'child-liam': INITIAL_CHILDREN[6],
  'child-leo': INITIAL_CHILDREN[2],
  'child-nick': INITIAL_CHILDREN[3],
...
function childIdFor(child: Child, index: number) {
  ...
  if (!child.id && child.name === 'Leo') return 'child-leo';
  if (!child.id && child.name === 'Nick') return 'child-nick';
